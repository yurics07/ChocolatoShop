...
// --- FUNÇÃO DE BUSCA (MODIFICADA PARA USAR CLASSE NO CONTAINER) ---
function performSearch() {
    const searchInput = document.getElementById('search-input');
    const searchTerm = searchInput.value.trim().toLowerCase();
    const productsContainer = document.querySelector('.products'); // Seleciona o container
    const products = productsContainer.querySelectorAll('.product-card');
    // Não precisamos mais selecionar as imagens aqui
    let foundMatch = false;

    removeNoResultsMessage(productsContainer);

    // Se o termo de busca estiver vazio, mostra todos os produtos e remove a classe do container
    if (searchTerm === "") {
        productsContainer.classList.remove('search-active'); // <<-- REMOVE A CLASSE DO CONTAINER
        products.forEach(product => {
            product.style.display = 'flex';
        });
        // REMOVIDO: Linhas que mexiam na classe 'product-image-shrunk' das imagens
        return; // Termina a função aqui
    }

    // --- Busca ativa ---
    productsContainer.classList.add('search-active'); // <<-- ADICIONA A CLASSE AO CONTAINER

    // Itera para mostrar/esconder os cards
    products.forEach(product => {
    ...
    if (!foundMatch) {
        // Se não encontrou nada, ainda mantém a classe 'search-active' para o estilo reduzido
        displayNoResultsMessage(productsContainer, searchInput.value.trim());
    }
}
// --- FIM DA FUNÇÃO DE BUSCA MODIFICADA ---
...
// Inicializar o carrinho e o Swiper ao carregar a página (SUBSTITUÍDO)
